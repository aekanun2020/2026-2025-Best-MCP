...
@mcp_server.tool()
async def search_documentation(query: str, limit: int = 5, search_mode: str = "hybrid") -> str:
    ...
    try:
        ...
        logger.info(f"Searching documentation with query: {query}, mode: {search_mode}")
        
        # Use SearchOrchestrator if available (hybrid search), otherwise fall back to semantic
        if search_orchestrator is not None:
            results = await search_orchestrator.search(query, limit, search_mode)
        else:
            # Fallback to semantic-only search
            logger.warning("SearchOrchestrator not available, using semantic-only search")
...
@mcp_server.tool()
async def add_context(content: str, title: str = None, source: str = "agent_context", metadata: dict = None) -> str:
    """Add context content directly to the RAG database
    
    Args:
        content: The text content to add to the database
        title: Optional title for the content (default: auto-generated)
        source: Source identifier (default: "agent_context")
    ...
    try:
        ...
        logger.info(f"Adding context content to database: {len(content)} characters")
        ...
        doc_metadata = DocumentMetadata(
            source=source,
            title=title or f"Context added at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            url=None,
        ...
        try:
            ...
            chunks = await text_processor.process_content(temp_file_path)
            
            if not chunks:
                return "Error: No content could be extracted from the provided text"
            ...
            if search_orchestrator is not None:
                try:
                    await search_orchestrator.bm25_retriever.add_documents(chunks)
                    logger.info(f"BM25 index updated with {len(chunks)} chunks from context")
                except Exception as e:
                    logger.warning(f"Failed to update BM25 index: {e}")
                    # Continue even if BM25 update fails
            
            logger.info(f"Successfully added context: {len(chunks)} chunks")
            
            return f"Successfully added context to database:\n" \
        ...
        error_msg = f"Error adding context: {str(e)}"
        logger.error(error_msg, exc_info=True)
        return error_msg
            ...
            for filename in files:
                ...
                try:
                    ...
                    if search_orchestrator is not None:
                        try:
                            await search_orchestrator.bm25_retriever.add_documents(chunks)
                            logger.info(f"BM25 index updated with {len(chunks)} chunks from {file_path}")
                        except Exception as e:
                            logger.warning(f"Failed to update BM25 index: {e}")
                            # Continue even if BM25 update fails
