"""
Database connection and utilities (MySQL / MariaDB)
"""
import os
import logging
...
def get_engine() -> Engine:
    ...
    global engine
    if engine is None:
        # PyMySQL driver. charset=utf8mb4 for full Unicode (incl. emoji / Thai).
        connection_string = (
        ...
        engine = create_engine(
            connection_string,
            pool_pre_ping=True,   # validate connections before use (handles MySQL wait_timeout)
            pool_recycle=3600,
        )
...
def get_tables() -> List[str]:
    """Get all base-table names in the current database"""
    conn = get_connection()
    if not conn:
        return []
...
def get_table_schema(table_name: str) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        for row in result:
            col_name, data_type, max_length, is_nullable, default, col_key, extra = row
            column_info = {
                "name": col_name,
                "type": data_type,
                ...
                "default": default,
                "key": col_key,          # PRI / UNI / MUL / '' (MySQL-specific)
                "extra": extra,          # e.g. auto_increment (MySQL-specific)
            }
...
def get_database_info() -> Dict[str, Any]:
    ...
    try:
        ...
        # Get database size (sum of data + index length) in MB
        query_size = text("""
        ...
        info["size_mb"] = round(float(size_mb), 2) if size_mb is not None else None
...
def get_all_relationships() -> List[Dict[str, Any]]:
    """Get all foreign key relationships in the current database"""
    conn = get_connection()
    if not conn:
        return []
...
def get_loner_tables() -> List[str]:
    ...
    A "loner" table neither references another table nor is referenced by one.
    """
    conn = get_connection()
    if not conn:
        return []
        ...
        query = text("""
            ...
            WHERE t.TABLE_TYPE = 'BASE TABLE'
              ...
              AND t.TABLE_NAME NOT IN (
                    -- tables that reference another table (have an outgoing FK)
              ...
              AND t.TABLE_NAME NOT IN (
                    -- tables that are referenced by another table (incoming FK)
                    SELECT DISTINCT kcu.REFERENCED_TABLE_NAME
                    FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS kcu
                    WHERE kcu.TABLE_SCHEMA = DATABASE()
                      AND kcu.REFERENCED_TABLE_NAME IS NOT NULL
              )
