"""
MCP Server implementation with Streamable HTTP transport (MySQL backend)
"""
import json
import logging
...
mcp_server = FastMCP("mysql-streamable-http-server")

# Initialize Streamable HTTP transport
# The MCP endpoint is exposed at "/mcp" by main.py. is_json_response_enabled=True
# makes POSTed JSON-RPC requests return a single application/json response.
...
# MCP Resources
@mcp_server.resource(uri="mysql://tables")
...
@mcp_server.resource(uri="mysql://schema/{table_name}")
...
async def execute_query_tool(query: str) -> str:
    """Run an arbitrary MySQL query and return the result rows as text.
    ...
    ARGS:
        query (str): A single MySQL statement. Must be non-empty.
            Use MySQL syntax: LIMIT n (not SELECT TOP n), NOW()/CURDATE() for the
            current date, backticks `col` for quoting identifiers.
            This server runs against MySQL / MariaDB.
...
async def preview_table(table_name: str, limit: int = 10) -> str:
    """Peek at a few sample rows from a table (SELECT * FROM table LIMIT n).
    ...
    ARGS:
        table_name (str): Exact table name as returned by get_database_context /
            list_tables. Must be non-empty. Wrapped in `backticks` automatically.
    ...
    # Escape backticks inside the identifier, then quote with backticks (MySQL).
...
async def get_database_info_tool() -> str:
    ...
    WHEN TO USE:
        - When you need the database name, server, table count, MySQL
    ...
    RETURNS (JSON string):
        {"database_name", "server", "port", "table_count", "server_version", "size_mb"}.
...
async def get_database_context() -> str:
    """Get the full database structure: schemas, relationships, and a MySQL guide.
    ...
    RETURNS (JSON string), an object with:
        ...
        - mysql_syntax_guide: MySQL reminders (LIMIT not TOP, NOW not GETDATE, etc.)
        - relationships: {has_relationships, relationship_count, foreign_keys[],
          loner_tables[], query_hints}
        - table_schemas: per-table column list (name, type, max_length, nullable,
          default, key, extra)
    ...
    context = {
        ...
        "mysql_syntax_guide": {
            "limit_rows": "SELECT * FROM table LIMIT 10 (NOT SELECT TOP 10)",
            "current_date": "NOW() or CURDATE() (NOT GETDATE())",
            "string_concat": "CONCAT(a, b) (NOT a + b)",
            "date_format": "DATE_FORMAT(col, '%Y-%m-%d')",
            "identifier_quote": "Use backticks: `column name`",
            "auto_increment": "AUTO_INCREMENT (NOT IDENTITY)"
        },
        ...
        "common_queries": {
            ...
            "get_schema": (
                ...
                "WHERE TABLE_NAME='YourTable' AND TABLE_SCHEMA=DATABASE()"
            )
        }
    }
    ...
    if relationship_graph["has_relationships"]:
        context["mysql_syntax_guide"]["foreign_keys"] = (
            "SELECT * FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE "
            "WHERE REFERENCED_TABLE_NAME IS NOT NULL AND TABLE_SCHEMA=DATABASE()"
        )
        context["mysql_syntax_guide"]["joins"] = "Use proper JOIN syntax with ON clause"
        context["common_queries"]["view_fks"] = (
...
async def generate_sql_query(description: str) -> Dict:
    ...
    query_prompt = f"""
I need help generating a SQL query for a MySQL database (MySQL syntax).
...
Remember:
- Use proper MySQL syntax (LIMIT not TOP, NOW() not GETDATE(), backticks for identifiers)
